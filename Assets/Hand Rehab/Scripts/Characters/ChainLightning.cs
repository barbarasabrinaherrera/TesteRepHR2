...
public class ChainLightning : MonoBehaviour
{
    ...
    public void LightItUp(Vector3 handPalmPositionVector, Vector3 handPalmNormalVector)//Hand hand)
    {

        GameObject firstEnemy = FindFirstEnemy(handPalmPositionVector, handPalmNormalVector, range);//hand, range);
        if (firstEnemy != null)
        {
            GameObject nextEnemy = firstEnemy;
            Vector3 currentPosition;
            Vector3 nextPosition = handPalmPositionVector;//hand.PalmPosition.ToVector3();
        ...
        { // miss, but cast lightning anyway
            GameObject lightning = GameObject.Instantiate(lightningBolt);
            var lightningPositions = lightning.GetComponentsInChildren<Transform>();
            lightningPositions[1].position = handPalmPositionVector;//hand.PalmPosition.ToVector3();
            lightningPositions[2].position = handPalmPositionVector + handPalmNormalVector * 10;//hand.PalmPosition.ToVector3() + hand.PalmNormal.ToVector3() * 10;
            Destroy(lightning, 3);
        }
    ...
    GameObject FindFirstEnemy(Vector3 handPalmPositionVector, Vector3 handPalmNormalVector, float range)//Hand hand, float range)
    {
        float squaredRange = range * range;
        Vector3 startPosition = handPalmPositionVector;//hand.PalmPosition.ToVector3();
        GameObject closestEnemy = null;
        if (targets.Count > 0)
        {
            ...
            {
                var enemiesInFrontOfMe = closeEnemies.Where(t => Vector3.Angle(t.transform.position - startPosition, handPalmNormalVector) < 30).ToList();//hand.PalmNormal.ToVector3()) < 30).ToList();
